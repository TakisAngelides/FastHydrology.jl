...
- **`KazmierczakHydroModel`** -- the fast, simplified subglacial hydrology model of
  ...
  via one of three interchangeable flux-routing implementations, selected with the
  `psi_out_algorithm` keyword: `RecursivePsiOut()` (the default) is substantially faster, but its
  native call-stack usage can overflow on large/convoluted real domains depending on the calling
  process's stack size; `IterativePsiOut()` trades that speed for a heap-allocated stack with no
  such limit; `TopologicalPsiOut()` is a single-pass topological sort (Kahn's algorithm) that's
  exact only when the flow-direction graph is acyclic -- true on idealized synthetic grids, but
  confirmed not reliably true on real ice-sheet data, so it errors by default (rather than silently
  returning a wrong field) whenever a cycle is detected. It matches that flux to a local conduit
  scale, and parameterizes the effective pressure for both hard and soft beds with an automatic
  switch between efficient (channelized) and inefficient (distributed) drainage. The reportable
  water layer thickness `state.W` has three selectable closures, chosen with the
  `water_thickness_algorithm` keyword: `DarcyWeisbachThickness()` (the default -- the turbulent
  analogue of the laminar Le Brocq/Weertman closure, consistent with the model's own turbulent-flow
  assumption), `ArealConduitThickness()` (the conduit cross-section smeared over the inter-conduit
  spacing), and `LaminarThickness()` (kept for direct comparison against Kori-ULB's/SHAKTI's own
  laminar sheet closure, but not recommended as "the" reported thickness since it's physically
  inconsistent with the model's turbulent-flow assumption) -- see the
  [API reference](https://TakisAngelides.github.io/FastHydrology.jl/dev/API_public/) for the
  physics behind each and for the `gradient_convention` (`LocalGradient()`/`MeanGradient()`) the two
  sheet-flow closures additionally take. The water source feeding the routing
  algorithm includes, by default, the dissipation melt rate generated by the flux itself as it
  moves down the hydraulic gradient -- a term the paper drops as negligible but which is kept here
  for generality (toggle with the `dissipation_melt` keyword). It can also include a
  frictional-heating term from basal sliding (Eq. 3 of the paper) via the `sliding_law` keyword
  (`AbstractSlidingLaw` and its subtypes -- see the
  [API reference](https://TakisAngelides.github.io/FastHydrology.jl/dev/API_public/) for details),
  two of which (`PowerPlasticSlidingLaw`, `RegularizedCoulombSlidingLaw`) are written to match the
  sliding laws implemented in [Yelmo.jl](https://github.com/palma-ice/yelmo) for coupled
  ice-dynamics use. Its numerical-safety bounds (`Wmin`/`Wmax`/`q_max`/`sigmat`) default to no-op
  ...
  and emits a warning explaining how to choose it (from grid resolution relative to ice thickness)
  if left unspecified.
