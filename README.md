...
- **`KazmierczakHydroModel`** -- the fast, simplified subglacial hydrology model of
  ...
  via either a recursive or an iterative (stack-based) implementation of the same algorithm,
  selected with the `psi_out_algorithm` keyword (`RecursivePsiOut()` by default -- substantially
  faster, but its native call-stack usage can overflow on large/convoluted real domains depending
  on the calling process's stack size; `IterativePsiOut()` trades that speed for a heap-allocated
  stack with no such limit). It matches that flux to a local conduit scale, and parameterizes the
  effective pressure for both hard and soft beds with an automatic switch between efficient
  (channelized) and inefficient (distributed) drainage. The water source feeding the routing
