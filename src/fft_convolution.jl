...
Only symmetric kernels are supported (convolution == correlation for those),
...
mutable struct FFTConvCache
    frb::Int                        # kernel radius: kernel is (2*frb+1, 2*frb+1)
    full_size::Tuple{Int,Int}       # padded array size, (Nx+2*frb, Ny+2*frb)
...
Build a new `FFTConvCache` for an `(Nx, Ny)` image convolved with a kernel of radius `frb`,
allocating the padded buffers and FFTW plans once so later calls (via `get_fft_conv_cache!`) only
pay for the FFT/multiply/inverse-FFT work itself.
"""
function FFTConvCache(Nx::Int, Ny::Int, frb::Int)
    full_size = (Nx + 2 * frb, Ny + 2 * frb) # frb cells of border padding on every side
...
Copy `src` into the interior of `padded`, then fill the surrounding `frb`-cell border with
"replicate" padding (nearest edge/corner pixel extended outward), matching `imfilter!`'s default
border behavior. `padded` must already be sized `(size(src,1)+2*frb, size(src,2)+2*frb)`.
"""
function fill_padded_src!(padded::Matrix{Float64}, src::AbstractMatrix, frb::Int)
    Nx, Ny = size(src)
    padded[frb+1:frb+Nx, frb+1:frb+Ny] .= src # interior = the real data, unchanged
...
Embed the centered `kernel2d` (indexed `[di+frb+1, dj+frb+1]` for offset `(di, dj)` from its
...
function fill_padded_kernel!(padded::Matrix{Float64}, kernel2d::AbstractMatrix, frb::Int, full_size::Tuple{Int,Int})
    ...
    for dj in -frb:frb, di in -frb:frb
        v = kernel2d[di+frb+1, dj+frb+1] # kernel value at offset (di, dj) from its center
...
function cached_fft_convolve!(cache_ref::Base.RefValue, dest::AbstractArray, src::AbstractArray, kernel::AbstractArray)
    ...
    frb = (size(kernel_p, 1) - 1) ÷ 2 # kernel is (2*frb+1, 2*frb+1, 1); recover frb from its size
    Nx, Ny = size(src_p, 1), size(src_p, 2)
    c = get_fft_conv_cache!(cache_ref, Nx, Ny, frb) # (re)allocates only if size/frb changed since the last call
    ...
    # crop back to the original (Nx, Ny) region, discarding the frb-cell border used for padding
