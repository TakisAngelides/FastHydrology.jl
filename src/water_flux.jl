...
The water source that feeds the routing algorithm is not just the externally-supplied basal melt mdot, but also the dissipation melt rate ṁ_w = |q * grad(phi0_s)| / L_w generated by the flux
itself as it moves down the hydraulic gradient (Eq. (3), Sec. 2.2.2 of Kazmierczak et al 2024). The paper drops this term as negligible; we keep it here for generality. Since it depends on q,
which is itself the output of the routing algorithm below, we resolve the resulting fixed point with a few Picard iterations -- the term is small enough (by the paper's own account) that this
...
function update_q!(model::KazmierczakHydroModel, grid::AbstractHydroGrid, state::HydroState)
    ...
    # gradients, so it stays fixed across the Picard iterations below.
    dx = grid.dx
    dy = grid.dy
    ...
    n_dissipation_iters = 3
    for _ in 1:n_dissipation_iters
        ...
        @. model.mdot_total = model.mdot + abs(model.q * model.abs_grad_phi0_s) / model.L_w
