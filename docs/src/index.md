...
- [`KazmierczakHydroModel`](@ref) -- the fast, simplified subglacial hydrology model of
  ...
  via one of three interchangeable flux-routing implementations -- see [Water Flux](@ref) for the
  `psi_out_algorithm` keyword (`RecursivePsiOut()`/`IterativePsiOut()`/`TopologicalPsiOut()`) and
  the trade-offs between them. It matches that flux to a local conduit scale, and parameterizes the
  effective pressure for both hard and soft beds with an automatic switch between efficient
  (channelized) and inefficient (distributed) drainage. The reportable water layer thickness
  `state.W` has three selectable closures -- see [Water Flux](@ref) for the
  `water_thickness_algorithm` keyword (`DarcyWeisbachThickness()`/`ArealConduitThickness()`/
  `LaminarThickness()`) and the `gradient_convention` the two sheet-flow closures additionally
  take. The water source feeding the routing
  ...
  see [Sliding Laws](@ref), two of whose laws (`PowerPlasticSlidingLaw`,
  `RegularizedCoulombSlidingLaw`) are written to match the sliding laws implemented in
  [Yelmo.jl](https://github.com/palma-ice/yelmo) for coupled ice-dynamics use. Its numerical-safety
  bounds (`Wmin`/`Wmax`/`q_max`/`sigmat`) default to no-op values -- nothing is clamped unless you
  ask for it -- with KORI-ULB's own values available as explicit opt-ins; `longcoupwater` has no
  such safe no-op default and emits a warning explaining how to choose it if left unspecified.
