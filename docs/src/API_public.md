...
- [`KazmierczakHydroModel`](@ref) -- the fast, simplified model of
  ...
  [Sliding Laws](@ref). The flux-routing sweep itself has two interchangeable implementations,
  selected with the `psi_out_algorithm` keyword -- see [Water Flux](@ref).
...
al. 2009's flow-routing algorithm -- has two implementations that produce identical results
([`accumulate_psi_out!`](@ref)/[`update_psi_out!`](@ref), the original recursive version, and
[`update_psi_out_iterative!`](@ref), a stack-based rewrite), verified to match bit-for-bit
(including `max_psi_out_calls` cutoff behaviour) on synthetic grids and on real Thwaites and
pan-Antarctica datasets. [`route_psi_out!`](@ref) is what `resolve_q!` actually calls each sweep;
it dispatches on `model.psi_out_algorithm` ([`AbstractPsiOutAlgorithm`](@ref), set via the
`psi_out_algorithm` keyword on [`KazmierczakHydroModel`](@ref)) to pick between them.
[`RecursivePsiOut`](@ref) (the default) is substantially faster (~30-40x, benchmarked on Thwaites
2km and pan-Antarctica 8km) but recurses as deep as the domain's longest flow-routing chain, which
on real (non-toy) grids can exceed the calling process's stack size and crash with a
`StackOverflowError` -- independent of, and potentially before, `max_psi_out_calls` ever binds.
[`IterativePsiOut`](@ref) has no such requirement (its "stack" is a heap-allocated `Vector`) at the
cost of that slowdown; prefer it for large/convoluted domains or when running under a constrained
or unknown stack limit (CI, HPC job schedulers, etc.).
